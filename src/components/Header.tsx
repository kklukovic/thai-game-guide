...
const Header = ({ language = "th" }: { language?: Language }) => {
      ...
      <div className="hero-shell">
        <div className="hero-copy-block">
          ...
          <h1 className="mt-5 max-w-3xl font-display text-[34px] font-semibold leading-[1.08] tracking-[-0.035em] text-foreground sm:text-[42px] md:text-[54px] lg:text-[62px]">
            {en ? "5 online casinos on Pim's Thailand shortlist for 2026" : "5 คาสิโนออนไลน์ที่เชื่อถือได้มากที่สุดในไทย 2026"}
          </h1>

          <p className="mt-5 max-w-2xl font-thai text-[15px] leading-7 text-muted-foreground md:text-[17px] md:leading-8">
            {en
              ? "A simple comparison of the main offers, payment options and details Pim has checked in one place."
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            <div className="hero-trust-pill">
              ...
              <span className="font-thai">{en ? "Reviewed and updated by Pim" : "ตรวจสอบและยืนยันโดยพิมแล้ว"}</span>
            </div>
          </div>
          ...
          <div className="mt-8 grid max-w-xl grid-cols-3 gap-3 border-t border-border/70 pt-6">
            <div>
              ...
              <div className="mt-1 font-thai text-[11px] leading-5 text-muted-foreground">{en ? "main picks" : "รายการหลัก"}</div>
            </div>
            <div>
              <div className="font-display text-xl font-semibold text-foreground">TH / EN</div>
              <div className="mt-1 font-thai text-[11px] leading-5 text-muted-foreground">{en ? "two languages" : "สองภาษา"}</div>
            </div>
            <div>
              <div className="font-display text-xl font-semibold text-foreground">2026</div>
              <div className="mt-1 font-thai text-[11px] leading-5 text-muted-foreground">{en ? "latest update" : "อัปเดตล่าสุด"}</div>
            </div>
          </div>
        </div>
          ...
          <div className="relative z-10 flex h-full flex-col">
            <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-5">
              <div>
                ...
                <h2 className="mt-2 font-display text-2xl font-semibold tracking-[-0.02em] sm:text-3xl">
                  {en ? "The useful details, all on one page" : "ทุกอย่างที่สำคัญ อยู่ในหน้าเดียว"}
                </h2>
              </div>
            ...
            <div className="mt-6 space-y-3">
              <div className="flex gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div>
                  <p className="font-display text-base font-medium">{en ? "Current offers" : "อัปเดตโปรล่าสุด"}</p>
                  <p className="mt-1 font-thai text-[12px] leading-5 text-white/60">
                    {en ? "The main offer details are kept short and easy to scan." : "สรุปเฉพาะข้อมูลหลักให้อ่านง่าย ไม่ต้องไล่เปิดหลายหน้า"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div>
                  <p className="font-display text-base font-medium">{en ? "5 clear comparisons" : "5 รายการแบบชัด ๆ"}</p>
                  <p className="mt-1 font-thai text-[12px] leading-5 text-white/60">
                    {en ? "Offers, highlights and payment methods use the same simple format." : "เรียงรายละเอียด โบนัส และจุดเด่นไว้ในรูปแบบเดียวกัน"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div>
                  <p className="font-display text-base font-medium">{en ? "Pim's Telegram" : "Telegram ของพิม"}</p>
                  <p className="mt-1 font-thai text-[12px] leading-5 text-white/60">
                    {en ? "Short updates and casual conversation outside email." : "สำหรับอัปเดตสั้น ๆ และพูดคุยกันแบบสบาย ๆ"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-auto pt-6">
              ...
              >
                {en ? "See all picks" : "ดูรายการทั้งหมด"}
                <span aria-hidden>↓</span>
              </a>
              <p className="mt-4 font-thai text-[11px] text-white/45">{en ? "Page updated: September 2026" : "อัปเดตหน้า: กันยายน 2026"}</p>
            </div>
          </div>
        </div>
      </div>
